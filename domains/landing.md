...
- **Logged Out Member** — a gated home, a placeholder label ("Home as a hub (gated)"), not the splash. (The "Welcome back… you're not logged in" welcome card was removed 2026-08-11; that copy now lives only in the panel access card — see [navigation.md](../foundation/navigation.md) §3.)
...
- **Hero avatar stack:** 24px circles with a `2px solid #6b3794` ring (Figma ring colour — a dark navy ring read as an unwanted border on the gradient), overlapped `-8px`. Uses generic stock faces (`assets/community-{1..3}.png`) representing community members — **not** the named committee headshots (those are real advisors and shouldn't stand in for anonymous users).
